                ...
                <div class="table-responsive">
                    <table class="table" id="allProjects">
                        ...
                        <tr>
                            ...
                            @endif
                            <th scope="col">Unit Cost <br>(Manager)</th>
                            <th scope="col">Total Cost <br>(Manager)</th>
                            <th scope="col">Approve Total<br> Cost</th>
                            <th scope="col">Approve Cost</th>
                            <th scope="col">Reusable</th>
                            <th scope="col">Approve<br>Status</th>
                            @if(Auth::user()->isAdmin() || Auth::user()->isAccountant())
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($trans = $inv->itemLogs()->where('il_project_from', '=', $project->project_id)->get())
                                        {{ number_format($inv->itemLogs()->where('il_project_id', '=', $project->project_id)->sum('il_quantity') - $trans->sum('il_quantity'),2) }}
                                    @else
                                        {{ number_format($inv->itemLogs()->where('il_project_id', '=', $project->project_id)->sum('il_quantity'),2) }}
                                    @endif
                                </td>
                                @if(Auth::user()->isAdmin() || Auth::user()->isAccountant())
                                    <td>
                                ...
                                <td>
                                    @if($trans = $inv->itemLogs()->where('il_project_from', '=', $project->project_id)->get())
                                        {{ number_format($inv->itemLogs()->where('il_project_id', '=', $project->project_id)->max('il_per_cost') - $trans->max('il_per_cost'),2) }}
                                    @else
                                        {{ number_format($inv->itemLogs()->where('il_project_id', '=', $project->project_id)->max('il_per_cost'),2) }}
                                    @endif
                                </td>
                                <td>
                                    @if($trans = $inv->itemLogs()->where('il_project_from', '=', $project->project_id)->get())
                                ...
                                <td>
                                    {{ number_format($inv->final_price_up,2) }}
                                </td>
                                <td>
